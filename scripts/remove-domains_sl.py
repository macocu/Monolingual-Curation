...
dset=dataset('/data/monolingual/sl-pre-domain-removal.prevert')

# Open the final file to which we will write the new corpus
final_file = open("/data/monolingual/sl.prevert", "w")

# Load the list of porn domains, created with the porn_identifier_final.py (folder Porn-Identifier)
with open("sl/list-porn-domains-sl.json") as porn_file:
...
# Use only domains that are from the Slovene corpus
...
print(f"The number of domains on the list to be removed: {len(final_set_bad_domains)}")
